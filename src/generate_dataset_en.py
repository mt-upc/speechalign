...
def postprocess_wav(wav_list: List[float], sentence: str) -> List[float]:
  ...
  punctuation_marks = Counter(sentence)['.'] + Counter(sentence)['!'] + Counter(sentence)['?'] + Counter(sentence)['...']
...
def sent_pho_dur_to_word_tuples(sent_pho_dur: List[Tuple[str, int]]) -> List[Tuple[str, int]]:
    word_duration = 0
    current_word = ""
    word_tuples = []
    for i in range(len(sent_pho_dur)):
        ...
        if i == len(sent_pho_dur)-1:
            if sent_pho_dur[i][0] != '<BLNK>': current_word += sent_pho_dur[i][0] # We omit <BLNK> when saving the word phonemes
        ...
        # Other elements
            ...
            # Case: no word spacing (all phonemes)
                ...
                word_duration += sent_pho_dur[i][1]
                if sent_pho_dur[i][0] != '<BLNK>': current_word += sent_pho_dur[i][0]
            # Case: word spacing (' ')
            else:
...
def word_tuples_to_ini_end_clips(word_tuples: List[Tuple[str, int]], wav: List[float], sample_rate: float) -> List[Tuple[float, float]]:
...
def find_idxs_two_phon_words_merged(words: List[str], en_tts: TTS) -> List[int]:
    ...
    for idx_word in range(len(words)):
        if idx_word != len(words)-1:
            two_consec_words = "".join(words[idx_word]) + ' ' + "".join(words[idx_word+1])
            tokenizer = en_tts.synthesizer.tts_model.tokenizer
            phon_words = tokenizer.decode(tokenizer.text_to_ids(two_consec_words, language="en")).replace('<BLNK>','')
            ...
            if len(phon_words) == 1 and len(prev_phon_words) != 1 and words[idx_word+1] != '-':
...
def find_idxs_three_phon_words_merged(words: List[str], en_tts: TTS) -> List[int]:
    ...
    for idx_word in range(len(words)-2):
        if idx_word != len(words)-2:
            three_consec_words = "".join(words[idx_word]) + ' ' + "".join(words[idx_word+1]) + ' ' + "".join(words[idx_word+2])
            tokenizer = en_tts.synthesizer.tts_model.tokenizer
            phon_words = tokenizer.decode(tokenizer.text_to_ids(three_consec_words, language="en")).replace('<BLNK>','')
            phon_words = phon_words.split(' ')
            if (len(phon_words) == 1 and
                len(prev_phon_words1) != 1 and len(prev_phon_words2) != 1 and
                (words[idx_word+1] != '-' or words[idx_word+2] != '-')):
                idx_list.append(idx_word)
...
def find_idxs_n_pos_multiple_phon_words(words: List[str], en_tts: TTS) -> List[int]:
    ...
    for idx_word in range(len(words)):
        tokenizer = en_tts.synthesizer.tts_model.tokenizer
        phon_word = tokenizer.decode(tokenizer.text_to_ids(words[idx_word], language="en")).replace('<BLNK>','')
        phon_word = phon_word.split(' ')
        if len(phon_word) != 1:
            # Case without %: numbers, hyphenated compound words... (e.g., 30, 34, sister-in-law...)
            if (phon_percentage not in phon_word[-1]): # Use 'in' (not ==) because phon_word[-1] can have a stacked 'punctuation mark'
    ...
    return idx_list, n_pos_extra_list  # Idxs of the words phonemized in multiple phonemized words (idx ref wrt Moses detokenized sentence)
...
def reorganize_clips_after_phonemization(dataset_ini_end_clips, idx_list, n_pos_extra_list, idx_copy_two, idx_copy_three) -> List[Tuple[float, float]]:
    ...
    if len(idx_list) > 0:
        ...
        for d in dataset_ini_end_clips:
            if len(new_dataset) == 0 or len(new_dataset) <= threshold or len(idx_queue) == 0:
...
def reorganize_clips_to_match_tokens(dataset_ini_end_clips, idx_punc, idx_pos_s) -> List[Tuple[float, float]]:
...
def main():
    ...
    en_tts_single.synthesizer.tts_model.duration_predictor.register_forward_hook(logw_hook)
    ...
    en_tts_multi.synthesizer.tts_model.duration_predictor.register_forward_hook(logw_hook)
    ...
    for i, sent in enumerate(sentences):
        ...
        for j in range(NUM_SPEAKERS + 1):
            ...
            for inp, out, lw in zip(textenc_in, textenc_out, logw):
                ...
                subsent_durations = get_durations(lw, out[-1], en_tts_multi)[0][0].tolist()
                durations.extend(subsent_durations)

            sent_pho_dur.append(
                [(idx2char[p], d) for p, d in zip(phonemes, durations)]
            )

            word_tuples = sent_pho_dur_to_word_tuples(sent_pho_dur[j])
            dataset_ini_end_clips.append(word_tuples_to_ini_end_clips(word_tuples, wav, sample_rate))

            idx_list, n_pos_extra_list = find_idxs_n_pos_multiple_phon_words(words, en_tts_multi)
            ...
            assert len(words_assert) == (len(dataset_ini_end_clips[j]) + len(idx_copy_list_two) + 2*len(idx_copy_list_three) - n_shifts), \
                "Wrong idx detection of conflictive phonemization cases"

            dataset_ini_end_clips[j] = reorganize_clips_after_phonemization(dataset_ini_end_clips[j], idx_list, n_pos_extra_list, idx_copy_list_two, idx_copy_list_three)
            assert len(words_assert) == len(dataset_ini_end_clips[j]), "Wrong processing of clips after sentence phonemization"

            idx_punc, idx_pos_s = get_idx_punct_pos_s_token(tokens_sent)
            dataset_ini_end_clips[j] = reorganize_clips_to_match_tokens(dataset_ini_end_clips[j], idx_punc, idx_pos_s)
            ...
            sent_dic[f'{i:03d}'][spk_name] = [(word, cli[0], cli[1]) for (word, cli) \
                            in zip(tokens_sent, dataset_ini_end_clips[j])]
        ...
        with open(os.path.join(jsons_filepath, f'dataset_{i:03d}.json'), 'w', encoding='utf-8') as file:
